...
## TOPIC №1
**Если есть проблемы с доступом к подпискам GitHub через оригинальные ссылки** `https://raw.githubusercontent.com/igareck/` - **используйте альтернативно ссылки JSDelivr через CDN** `https://cdn.jsdelivr.net/`. 

**Ссылки на каждую подписку расположены в разделе** `🪞 ЗЕРКАЛА 🪞`. **Менять в них ничего не нужно - просто замените ссылки у себя в клиенте и продолжайте пользоваться подпиской. В ближайшее время в дополнение к JSDelivr выложу список разнообразных альтернатив.**
